...
### `POST /api/analyze/` Body (선택)

| 필드 | 설명 |
|------|------|
| `pipeline` | `research`, `graph`, `self_rag`, `crag_judge` — bool. 생략 시 `.env` 기본값(`DEPLOY_*_ENABLED` 등). |
| `include_pipeline_summary` | `true`이면 마지막 NDJSON 줄에 `pipeline_summary`(플래그·단계별 ms·RAG/그래프/Self-RAG 요약) 포함. |

`DeployReportRecord.extra`에는 해당 실행 기준으로 `pipeline_flags`, `pipeline_timings_ms`, `pipeline_summary` 등이 포함됩니다.
