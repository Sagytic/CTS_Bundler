...
class AnalyzeGuardianView(APIView):
    ...
    def post(self, request: Request) -> StreamingHttpResponse:
        ...
        def node_fetch_and_score(state: AgentState):
            progress["step"] = "fetch_data"
            progress["label"] = "1차 Rule-based 리스크 스코어링 중..."
            print("\n===========================================")
            print("🚀 [System] 배포 심의 위원회 소집 및 데이터 추출 시작")
            print("===========================================")

            sap_result = fetch_recent_transports_via_http(user_id=user_id)
            tr_data = []
            reasons = []
            score = 0
            deploy_grade = "Low"
        ...
        def create_expert_node(module_name, role_desc, state_key):
            def expert_node(state: AgentState):
                ...
                progress["label"] = f"{module_name} 모듈 검토 중..."
        ...
        node_bc = create_expert_node("BC", "시스템 인프라, DB 락, 권한, 공통 클래스 및 SICF등 SAP BC 관련 내용을 책임집니다.", "bc_analysis")
        node_fi = create_expert_node("FI", "재무회계, 전표 처리 테이블 등 SAP FI 관련 내용을 책임집니다.", "fi_analysis")
        node_co = create_expert_node("CO", "관리회계 및 수익성 분석 등 SAP CO 관련 내용을 책임집니다.", "co_analysis")
        node_mm = create_expert_node("MM", "자재/구매/재고 관련 테이블과 로직 등 SAP MM 관련 내용을 책임집니다.", "mm_analysis")
        node_sd = create_expert_node("SD", "영업/판매 관련 기능과 오더 처리 등 SAP SD 관련 내용을 책임집니다.", "sd_analysis")
        node_pp = create_expert_node("PP", "생산 계획 및 제조 실행 등 SAP PP 관련 내용을 책임집니다.", "pp_analysis")

        def node_report_generator(state: AgentState):
            progress["step"] = "architect"
            progress["label"] = "수석 아키텍트가 최종 보고서 작성 중..."
        ...
        initial_state = {
            "user_input": user_input, "sap_data_raw": [], "rule_score": 0, "rule_reasons": [],
        ...
        def run_invoke():
            ...
            faux_req = SimpleNamespace(request_id=req_id)
            try:
                try:
                    ...
                    with get_openai_callback() as cb:
                        try:
                            result = app.invoke(
                                initial_state, {**config, "callbacks": [cb]}
                            )
        ...
        def stream_generator():
            ...
            last_sent = None
            while t.is_alive():
                cur = {"step": progress.get("step", ""), "label": progress.get("label", "")}
                if cur != last_sent:
                    last_sent = dict(cur)
